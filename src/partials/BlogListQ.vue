<template>
  <section class="relative">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      <div class="pt-10 pb-12 md:pt-14 md:pb-16">
        <!-- Page header -->
        ...
        <div class="max-w-sm mx-auto md:max-w-none">
          ...
          >
            <!-- 1st article -->
            <article v-for="(item,index) in blogArticles" :key="index" class="flex flex-col h-full" data-aos="fade-up">
              <header>
                ...
                <div class="mb-3">
                  <ul class="flex flex-wrap text-xs font-medium -m-1">
                    <li class="m-1">
                      <a
                        class="capitalize inline-flex text-center text-gray-100 py-1 px-3 rounded-full bg-teal-500 hover:bg-teal-600 transition duration-150 ease-in-out"
                        href="#0"
                        >{{item.yoast_head_json.og_type}}</a
                      >
                    </li>
                  </ul>
                </div>
              ...
              <div v-html="item.excerpt.rendered" class="text-lg text-gray-400 h-20 grow text-ellipsis overflow-hidden">
              </div>
              <footer class="flex items-center mt-4">
...
export default {
  ...
  created() {
    axios.get(`https://quiblog.it/wp-json/wp/v2/posts`)
        .then((response => {
          this.blogArticles = response.data.splice(0, 3);
